...
implementation-progress: "E1 stories 1.1 + 1.2 + 1.3 of 10 (workspace + canonical Event + TelemetryAdapter trait + Zeek adapter)"
---
# Anomaly Grid CLI
...
- **Implementation status (2026-04-28):** Stories 1.1 + 1.2 + 1.3 landed. Cargo workspace builds clean (Rust 1.95 stable), `ag --version` ships the required fields, release binary is 631 KB stripped. Canonical `Event` schema and `TelemetryAdapter` trait shipped with full serde round-trip tests, byte-stable JSON, object-safe trait, rustdoc doctest. **Zeek adapter** ships `conn.log` / `dns.log` / `ssl.log` / `http.log` ingestion in both TSV and JSON formats with auto-detection, `supported_versions = ["4.x", "5.x"]`, async streaming via `tokio::fs` + `async_stream::stream!` (no whole-dataset buffering per NFR13). 31 tests + 1 doctest passing workspace-wide. CI: fmt + strict clippy (`-D unwrap_used -D expect_used`) + test all green. Next: Story 1.4 (journald + syslog adapter).
